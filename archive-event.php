...
<div id="primary" class="site-content">
    <div id="content" role="main">
        ...
        <?php
            // Show past events when linking from the calendar widget to the archive for a specific day (BHM)
            if (eo_is_event_archive('day')) {
                global $query_string;
                $args = array_merge ($wp_query->query_vars, array('showpastevents' => 1));
                query_posts($args);
            }
        ?>
                ...
                <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
                    <header class="entry-header">
                        ...
                        <div class="event-entry-meta">
                            ...
                            <?php
                            //Format date/time according to whether it's an all-day event.
                            //Use microdata http://support.google.com/webmasters/bin/answer.py?hl=en&answer=176035
                            if( eo_is_all_day() ){
                                $format = 'l, F j, Y';
                                $microformat = 'Y-m-d';
                            }else{
                                $format = 'l, F j, Y \a\t ' . get_option('time_format');
                                $microformat = 'c';
                            }?>
