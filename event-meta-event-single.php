...
<div class="entry-meta eventorganiser-event-meta">
    ...
    <?php if( eo_is_all_day() ){
        ...
        $date_format = 'l, F j, Y \a\t ' . get_option('time_format');
    } ?>
    <hr>
    ...
    <?php if( eo_reoccurs() ):?>
        ...
        <?php if($next): ?>
            ...
            <?php printf('<p>'.__('This event is running from %1$s until %2$s. It is next occurring on %3$s','eventorganiser').'.</p>', eo_get_schedule_start('F j, Y'), eo_get_schedule_last('F j, Y'), $next);?>

        <?php else: ?>
            <!-- Otherwise the event has finished (no more occurrences) -->
            <?php printf('<p>'.__('This event finished on %s','eventorganiser').'.</p>', eo_get_schedule_last('F j, Y',''));?>
        <?php endif; ?>
    <?php endif; ?>
